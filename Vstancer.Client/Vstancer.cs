...
namespace Vstancer.Client
{
    public class Vstancer : BaseScript
    {
        ...
        private MenuDynamicListItem AddDynamicFloatList(Menu menu, string name, float defaultValue, float value, float maxEditing, float step = (float)0.0)
        {
...
// fSteeringLock
        ...
        private void SetVstancerPreset(int vehicle, float off_f, float rot_f, float off_r, float rot_r, float steeringLock, float suspensionRaise, object defaultFrontOffset = null, object defaultFrontRotation = null, object defaultRearOffset = null, object defaultRearRotation = null)
        {
            ...
            if (vehicle == currentVehicle)
            {
                currentPreset = new VstancerPreset(wheelsCount, rot_f, rot_r, off_f, off_r, rot_f_def, rot_r_def, off_f_def, off_r_def, steeringLock, suspensionRaise);
                BuildMenu();
            }
            else
            {
